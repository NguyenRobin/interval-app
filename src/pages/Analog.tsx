...
function Analog() {
  ...
  const [analogView, setAnalogView] = useState('');
  ...
  const totalSeconds = useSelector((state: GlobalState) => state.minToSec);
  const currentMinAnalog = useSelector(
    (state: GlobalState) => state.currentMinAnalog
  );

  const fullRotationDeg = 360;
  const fullRotationInSec = 60;
  const currentSecondAsDeg =
    (totalSeconds / fullRotationInSec) * fullRotationDeg;
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      if (totalSeconds % 60 === 0) {
        dispatch(updateCurrentMinAnalog(1));
      }
    }, 1000);
  ...
  useEffect(() => {
    ...
    setAnalogView(formatTime(seconds));
  ...
  }, [isRunning, dispatch, totalSeconds, navigate, isBreak, isInterval]);
  return (
    <div className={styles.Analog}>
      ...
      >
        ...
        <rect
          ...
          fill="black"
          style={{ rotate: `${currentMinAnalog}deg` }}
        />
        <ellipse
          cx="4.98563"
          cy="4.98578"
        ...
        <rect
          ...
          style={{ rotate: `${-currentSecondAsDeg}deg` }}
          fill="#FF0000"
        />
        <defs>
